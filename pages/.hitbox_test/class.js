            ...
            array.forEach((element) => {
                this.hitbox.forEach((box) => {
                    if(element.x != 0 && element.y != 0 && box.x != 0 && box.x != 0){
                        this.x = ( element.y < box.y + box.h && element.y + element.h > box.y && this.old_velx != 0) ? 
                            (this.old_velx < 0 && box.x + this.old_velx < element.x + element.w && this.move.old_x - box.w/2 >= element.x + element.w) ? 
                                element.x + element.w + box.w / 2 : //true
                            (this.old_velx > 0 && element.x < box.x + box.w + this.old_velx && this.move.old_x + box.w/2 <= element.x) ?
                                element.x - box.w / 2 : this.x:     //false
                            this.x  ;                               //else
...
class Player extends Objects{
    ...
    draw(){
        ...
        add_draw_array({z: this.y, draw : [document.querySelector('.josef1'), this.anm.frame_size[0] * this.anm.frame, this.anm.frame_size[1] * this.anm.rotation_dir, this.anm.frame_size[0] , this.anm.frame_size[1], this.x - (this.anm.frame_size[0] / 2), Math.round(this.y - (this.anm.frame_size[1] / 1.1)), this.anm.frame_size[0], this.anm.frame_size[1]]});
    }
...
class Placeable extends Objects{
    ...
    draw(){
        ...
        add_draw_array({z: this.y, draw: [document.querySelector(`.${this.type}`), this.anm.frame_size[0] * this.anm.frame, 0, this.anm.frame_size[0] , this.anm.frame_size[1], this.x - (this.anm.frame_size[0] / 2), this.y - (this.anm.frame_size[1] / 2), this.anm.frame_size[0], this.anm.frame_size[1]]});

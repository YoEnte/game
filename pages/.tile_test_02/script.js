...
Object.keys(maps).forEach(_map => {
	maps[_map].size_x = maps[_map].map.length;
	maps[_map].size_y = maps[_map].map[0].length;
});
...
const assets = {
	grass_tile: 'assets/grass_tile.png',
	path_tile: 'assets/path_tile.png',
	path_overlay: 'assets/grass_tile_overlay_test1.png',
}

const assets_num = Object.keys(assets).length;
var assets_loaded = 0;

canvas.width = maps[selected_map].size_x * tile_size;
canvas.height = maps[selected_map].size_y * tile_size;
canvas.style.width = `${canvas_size}px`;
canvas.style.height = `${canvas_size}px`;
const pixel_size = canvas_size / (maps[selected_map].size_x * tile_size);
...
function gameloop(){
	...
	maps[selected_map].map.forEach((row, y) => {
		maps[selected_map].map[y].forEach((col, x) => {
			if(col == 0){
				ctx.drawImage(document.querySelector('.grass_tile'), x * tile_size, y * tile_size);
			}else if(col == 1){
				ctx.drawImage(document.querySelector('.path_tile'), x * tile_size, y * tile_size);
				...
				ctx.drawImage(document.querySelector('.path_overlay'), overlay_index * tile_size, 0, tile_size, tile_size, x * tile_size, y * tile_size, tile_size, tile_size);
			}
		});
	});
...
window.addEventListener('mousemove', event => {
	...
	if((temp_mouse_x < 0 || temp_mouse_x > maps[selected_map].size_x * tile_size) || (temp_mouse_y < 0 || temp_mouse_y > maps[selected_map].size_y * tile_size)){
		mouse_x = undefined;
		mouse_y = undefined;
	}else{
...
window.addEventListener('mousedown', event => {
	...
	var tile_x = Math.floor(mouse_x / tile_size);
	var tile_y = Math.floor(mouse_y / tile_size);
